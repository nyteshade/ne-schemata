...
import { dynamicImport, supportsNativeTypeScript } from './dynamicImport.mjs'

import { existsSync, readFileSync } from 'node:fs'
import { readFile } from 'node:fs/promises'
import { resolve, parse, format } from 'node:path'
import { asyncTryCatch, ifParsedPath } from './utils/index.mjs'
...
export async function resolvedPaths(
  ...
  if (parsed.ext && existsSync(resolved)) {
  ...
  for (let ext of tryExts) {
    ...
    let tryTrue = existsSync(tryPath)
 ...
 * This is used by the require.extensions handler.
 *
...
function readAndParseGraphQL(filename) {
  try {
    ...
    console.error(`Error reading GraphQL file ${filename}:`, error)
    throw error
  }
}

/**
 * Adds the ability to `require` files ending in a `.graphql`, `.gql`, or
 * `.sdl` extension. The exports returned from such an import consist of
 * major values and one default value.
 *
 * Note: This is synchronous because require.extensions handlers must be
 * synchronous. For async loading, use importGraphQL() directly.
 *
 * values:
 *   astNode   - an ASTNode document object representing the SDL contents
 *               of the .graphql file contents. Null if the text is invalid
 *   resolvers - (loaded async separately) if there is an adjacent file with
 *               the same name, ending in .js/.ts
...
export function graphQLExtensionHandler(module, filename) {
  // require.extensions handlers MUST be synchronous
  // This is a fundamental limitation of Node.js CJS module loading
  try {
    const result = readAndParseGraphQL(filename)
    module.exports = result
  }
  catch (error) {
    // Clean up require cache on error
...
/**
 * Detects if the current environment is an ESM module context.
 * Since this module is now ESM-only, this always returns true.
 *
 * @return {boolean} Always true since this is an ESM module
 */
function isESMContext() {
  return true
}

/**
 * Registers the custom extension handlers for `.graphql`, `.sdl`, and `.gql`
 * files.
 *
 ...
 *
 * For ESM contexts, use dynamic imports with importGraphQL() or a loader hook.
 */
export function register() {
  if (isESMContext()) {
    console.warn(
      '[ne-schemata] register() is disabled in ESM contexts. ' +
      'Use importGraphQL() for .graphql file loading in ESM.'
    )
    return
  }
  ...
  if (typeof require === 'undefined' || !require.extensions) {
    console.warn(
      ...
      'require.extensions is not available.'
    )
    return
  }
...
/**
 * Deregisters the custom extension handlers for `.graphql`, `.sdl`, and
 * `.gql` files.
 */
