/**
 * @fileoverview Schemata - A String extension for GraphQL SDL/IDL manipulation
 *
 * This module provides the Schemata class which extends String to provide
 * convenient methods for working with GraphQL Schema Definition Language.
 *
...
import {
  ...
  resolvedPaths
} from './GraphQLExtension.mjs'

import { asyncTryCatch } from './utils/index.mjs'
import { extractResolverInfo } from './utils/resolverwork.mjs'
import { ExtendedResolverMap } from './ExtendedResolverMap.mjs'
import { ExtendedResolver } from './ExtendedResolver.mjs'
import { inline, dedent } from 'ne-tag-fns'
import { mergeResolvers } from './walkResolverMap.mjs'
...
import { fileExists, findNearestPackageJson, guessProjectRoot } from './dynamicImport.mjs'
...
export class Schemata extends String {
  ...
  mergeSDL(
    ...
    for (let rType of rAST.definitions) {
      ...
      case 'InterfaceTypeDefinition':
        ...
        // Handle interface implementation merge if needed
        if (rType.interfaces && rType.interfaces.length) {
          lType.interfaces = lType.interfaces || []
          for (const rInterface of rType.interfaces) {
            const exists = lType.interfaces.find(
              i => i.name.value === rInterface.name.value
            )
            if (!exists) {
              lType.interfaces.push(rInterface)
            }
          }
        }
        break
      ...
      default:
        combineTypeAndSubType('directives', lType, rType, conflictResolvers)
        combineTypeAndSubType('fields', lType, rType, conflictResolvers)
        // Handle interface implementations for object types
        if (rType.interfaces && rType.interfaces.length) {
          lType.interfaces = lType.interfaces || []
          for (const rInterface of rType.interfaces) {
            const exists = lType.interfaces.find(
    ...
    if (Object.keys(_scalarFns).length) {
      for (let typeName of Object.keys(_scalarFns)) {
        const scalarType = merged.schema.getType(typeName)
        if (scalarType && typeof scalarType === 'object') {
          // Store the merged scalar resolver config on the type
          // Modern approach: merge with existing config if present
          const existingConfig = scalarType._scalarConfig || scalarType
          const mergedConfig = Object.assign(
            {},
            typeof existingConfig === 'object' ? existingConfig : {},
            _scalarFns[typeName] || {}
          )
          scalarType._scalarConfig = mergedConfig
        }
      }
    }
  ...
  /**
   * Explicitly define Symbol.toPrimitive to ensure proper string coercion
   * in all contexts. This is critical when extending native String.
   *
   * @param {string} hint - The type hint for conversion ('string', 'number', 'default')
   * @return {string} returns the underlying SDL string
   */
  [Symbol.toPrimitive](hint) {
    if (hint === 'number') {
      return Number(this.sdl)
    }
    return this.sdl
  }
   ...
   * over and returned.
   */
  forEachField(fn, context, types = ALL, suppliedSchema = null) {
    let schema = suppliedSchema || this.schema
   ...
   * and classes.
   *
   * @return {object} the graphql module namespace
   */
  static get gql() {
    return graphql
  }
  ...
  #generateSchema() {
    ...
    try {
      ...
      // Parse SDL to AST first
      const ast = Class.parse(this.sdl, true, false)

      // Separate base definitions from extensions
      const baseDefinitions = ast.definitions.filter(def =>
        !def.kind.endsWith('Extension')
      )
      const extensionDefinitions = ast.definitions.filter(def =>
        def.kind.endsWith('Extension')
      )

      // Create AST for base schema
      const baseAst = { ...ast, definitions: baseDefinitions }

      // Build base schema using buildASTSchema for more control
      const buildASTSchema = Class.gql.buildASTSchema
      schema = buildASTSchema(baseAst, {
        assumeValidSDL: this.validSDL
      })

      this[MAP].set(wmkSchema, schema)

      // Apply extensions in order: interface, type, union, enum, scalar, input
      ...
      for (const extKind of extensionOrder) {
        const extensionsOfKind = extensionDefinitions.filter(d => d.kind === extKind)
        if (extensionsOfKind.length > 0) {
          const extAst = { ...ast, definitions: extensionsOfKind }
          try {
            schema = extendSchema(schema, extAst)
            this[MAP].set(wmkSchema, schema)
          }
          catch (error) {
            debug_log(`[get .schema] failed to handle ${extKind}`)
    ...
    if (resolvers) {
      forEachField(
        ...
        (
          ...
          // Handle root type field resolvers (flattened pattern)
          ...
          if (typeResolvers) {
            ...
            if (fieldResolver) {
              ...
              else if (fieldResolver && typeof fieldResolver === 'object') {
      ...
      // Apply type-level resolvers (isTypeOf, resolveType, scalars)
      forEachOf(
        schema,
        (type, typeName) => {
          const typeResolvers = resolvers?.[typeName]
          ...
          // Apply isTypeOf for object types
          if (typeResolvers.__isTypeOf && typeof typeResolvers.__isTypeOf === 'function') {
            if (type.isTypeOf !== undefined) {
              type.isTypeOf = typeResolvers.__isTypeOf
            }
          }

          // Apply resolveType for interface/union types
          if (typeResolvers.__resolveType && typeof typeResolvers.__resolveType === 'function') {
            if (type.resolveType !== undefined) {
              type.resolveType = typeResolvers.__resolveType
            }
          }

          // Apply scalar configuration
          if (type instanceof GraphQLScalarType) {
            const scalarConfig = typeResolvers
            if (scalarConfig.serialize) {
              type._scalarConfig = scalarConfig
            }
          }
        },
        null,
 ...
 *
 * This modern implementation handles:
 * - Field-level resolve and subscribe functions
 * - Type-level isTypeOf and resolveType functions
 * - Scalar serialize, parseValue, and parseLiteral
 * - Schema-level resolution patterns
 *
...
export function stripResolversFromSchema(schema) {
  ...
  // Walk all fields to extract field-level resolvers
  forEachField(
    schema,
    (
      type,
      ...
      // Extract both resolve and subscribe functions
      if (field.resolve && field.resolve !== defaultFieldResolver) {
        resolvers[typeName] = resolvers[typeName] || {}
        resolvers[typeName][fieldName] = field.resolve
      }

      // Extract subscription resolver if present
      if (field.subscribe) {
        resolvers[typeName] = resolvers[typeName] || {}
        if (typeof resolvers[typeName][fieldName] === 'function') {
          // Wrap both resolve and subscribe in a field config object
          resolvers[typeName][fieldName] = {
            resolve: resolvers[typeName][fieldName],
            subscribe: field.subscribe
          }
        }
        else if (typeof resolvers[typeName][fieldName] === 'object') {
  ...
  // Walk all types to extract type-level and scalar resolvers
  forEachOf(
    schema,
    (type, typeName, typeDirectives, _schema, context) => {
      ...
      if (type instanceof GraphQLScalarType && !isSpecifiedScalarType(type)) {
        ...
        // Extract scalar methods if they differ from defaults
        if (type.serialize && type.serialize !== GraphQLScalarType.prototype.serialize) {
          scalarConfig.serialize = type.serialize.bind(type)
        }
        if (type.parseValue && type.parseValue !== GraphQLScalarType.prototype.parseValue) {
          scalarConfig.parseValue = type.parseValue.bind(type)
        }
        if (type.parseLiteral && type.parseLiteral !== GraphQLScalarType.prototype.parseLiteral) {
          scalarConfig.parseLiteral = type.parseLiteral.bind(type)
        }

        // Only add scalar if it has custom methods
        if (scalarConfig.serialize || scalarConfig.parseValue || scalarConfig.parseLiteral) {
          resolvers[typeName] = scalarConfig
        }
      }
    },
    null,
  ...
  // Handle schema-level resolution
  const schemaConfig = schema.toConfig ? schema.toConfig() : {}
  if (schemaConfig.resolveField || schemaConfig.resolveType) {
    resolvers.__schema = resolvers.__schema || {}
    if (schemaConfig.resolveField) {
      resolvers.__schema.resolveField = schemaConfig.resolveField
    }
    if (schemaConfig.resolveType) {
      resolvers.__schema.resolveType = schemaConfig.resolveType
    }
  }
...
function combineTypeAndSubType(
  ...
  if (rType[subTypeName]) {
    for (let rSubType of rType[subTypeName]) {
      ...
      let index = lType[subTypeName].indexOf(lSubType)
