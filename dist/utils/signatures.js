...
var SIG = exports.SIG = Symbol["for"]('Signature');
...
function enableSignatures() {
  Object.defineProperty(Function.prototype, 'signature', {
    get: function get() {
      ...
      var cleanedSource = source.replace(/\/\/.*$/gm, '').replace(/\/\*[\s\S]*?\*\//gm, '');
      if (cleanedSource.startsWith('class')) {
        ...
        var constructorMatch = cleanedSource.match(/constructor\s*\(([\s\S]*?)\)\s*{/);
        if (constructorMatch) {
          // Replace all newlines and multiple spaces with a single space
          var params = constructorMatch[1].replace(/(\r\n|\n|\r|\s)+/gm, ' ').trim();
          signature += "(".concat(params, ")");
        }
      } else {
        // Match the function signature (excluding the "function" keyword)
        var functionMatch = cleanedSource.match(/(?:function\s+)?([\w$]+)\s*\(([\s\S]*?)\)\s*{/);
        if (functionMatch) {
          // Replace all newlines and multiple spaces with a single space
          var _params = functionMatch[2].replace(/(\r\n|\n|\r|\s)+/gm, ' ').trim();
          signature = "function ".concat(functionMatch[1], "(").concat(_params, ")");
        }
      }
