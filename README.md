...
Some of the most often applied tasks with Schemata are building a schema object, building an executable schema object, parsing it into `ASTNode` objects. Occasionally a given bit of Schemata is checked for validity.
...
- [x] Checking of the validity of Schemata as a `ASTNode` as well as a `GraphQLSchema`
- [x] Merge Schemata together via AST parsing
- [x] Merge multple GraphQLSchemas via AST parsing
- [x] Pare down one Schemata/Schema using another as a guide
...
### From and existing instance of Schemata
...
You can also create a Schemata instance from an existing instance of GraphQLSchema. The typeDefs or Schemata string will be generated using `printSchema()` from the base graphql module. The `.schema` value will be set explicitly to the value supplied in the case it is executable or has had other modifications performed on it. To prevent it being regenerated by `.executableSchema` should that property be accessed, a symbol property will be set on the schema to denote it should not be touched.
...
console.log(schemata) // the Schemata from the remote schema
```

### Using the `gql` template function

Alternatively, creating a Schemata string instance can be done using the `gql` template function. The function was named as many popular editors have a syntax highlighting theme that highlights GraphQL Schemata. It seemed to make a lot of sense to add the resulting string from this as an instance of `Schemata`
...
const sdlC = sdlA.mergeSchemata(sdlB)
...
- [Instance properties](#instance-properties)
  - [.ast](#inst-ast)
  - [.executableSchema](#inst-executable-schema)
  - [.graphiql](#inst-graphiql)
  - [.hasAnExecutableSchema](#inst-has-an-executable-schema)
  - [.hasFlattenedResolvers](#inst-has-flattened-resolvers)
  - [.resolvers](#inst-resolvers)
  - [.rootValue](#inst-root-value)
  - [.schema](#inst-schema)
  - [.sdl](#inst-sdl)
  - [.typeDefs](#inst-type-defs)
  - [.valid](#inst-valid)
  - [.validSchema](#inst-valid-schema)
  - [.validSchemata](#inst-valid-sdl)
...
- [Static properties](#static-properties)
  ...
  - [.ENUMS](#const-enums)
  - [.HIDDEN](#const-hidden)
  - [.INPUT_TYPES](#const-input-types)
  - [.INTERFACES](#const-interfaces)
  - [.ROOT_TYPES](#const-root-types)
...
![getter](https://github.com/nyteshade/ne-schemata/raw/master/assets/get-left-arrow-24.png) retrieves any internally stored `GraphQLSchema` instance generated thus far; if one does not exist, one will be generated
![setter](https://github.com/nyteshade/ne-schemata/raw/master/assets/set-right-arrow-24.png) sets the value of the internal `GraphQLSchema` storage variable
...
#### <a name="inst-valid-sdl"></a>.validSchemata [✯](#contents)

![getter](https://github.com/nyteshade/ne-schemata/raw/master/assets/get-left-arrow-24.png) if the Schemata this string represents can be parsed without error, true will be returned; false otherwise

#### <a name="inst-valid-schema"></a>.validSchema [✯](#contents)

![getter](https://github.com/nyteshade/ne-schemata/raw/master/assets/get-left-arrow-24.png) if the Schemata this string represents can be used to build a schema without error, true will be returned; false otherwise

#### <a name="inst-valid"></a>.valid [✯](#contents)

![getter](https://github.com/nyteshade/ne-schemata/raw/master/assets/get-left-arrow-24.png) returns true if both `.validSchemata` and `.validSchema` are both true
...
```
Iterates over the type map of either the internal schema or one created from the Schemata string this instance represents and then stored for later use. See above for the definition of the `ForEachOfResolver` callback signature. `types` is a bitmask made up of at least one of the constant properties on Schemata such as `Schemata.ALL` or `Schemata.TYPES` and optionally augmented with `Schemata.HIDDEN`
...
Iterates over all types on either the internal schema or one created from the Schemata string this instance represents and then stored for later use
...
Iterates over all input types on either the internal schema or one created from the Schemata string this instance represents and then stored for later use
...
Iterates over all the unions on either the internal schema or one created from the Schemata string this instance represents and then stored for later use
...
Iterates over all enums on either the internal schema or one created from the Schemata string this instance represents and then stored for later use
...
Iterates over all interfaces on either the internal schema or one created from the Schemata string this instance represents and then stored for later use
...
Iterates over all scalars on either the internal schema or one created from the Schemata string this instance represents and then stored for later use
...
Iterates over the three root types (Query, Mutation and Subscription) on either the internal schema or one created from the Schemata string this instance represents and then stored for later use
...
Iterates over all type fields on all types on either the internal schema or one created from the Schemata string this instance represents and then stored for later use.
...
Iterates over all interface fields on all types on either the internal schema or one created from the Schemata string this instance represents and then stored for later use
...
Iterates over all input object fields on all types on either the internal schema or one created from the Schemata string this instance represents and then stored for later use.

*See above for the definition of the `ForEachFieldResolver` callback signature*

#### <a name="merge-sdl"></a>mergeSchemata [✯](#contents)
```js
mergeSchemata(
  schemaLanguage: string | Schemata,
  conflictResolvers: ?ConflictResolvers = DefaultConflictResolvers
): Schemata {
```

Given a string of Schema Definition Language (or Schemata) or a Schemata instance itself, the function will proceed to blend the supplied Schemata with the one of the instance. In the case of conflicts, either the default resolver, which simply takes the newer right hand value, or a function of your own devising will be called to decide. See the [`ConflictResolvers`](#conflict-resolvers) type for more info
...
This process stores the resolvers of the internal schema as well as those of the supplied schema. The supplied schema is then converted to Schemata and the mergeSchemata() function is applied. Post merge, the previously stored and merged resolvers map are are applied and a new executable schema is built from the ashes of the old. See the [`ConflictResolvers`](#conflict-resolvers) type for more info
...
#### <a name="type-directive-merge-resolvers"></a>DirectiveMergeResolver [✯](#contents)
```js
...
#### <a name="type-enum-merge-resolvers"></a>EnumMergeResolver [✯](#contents)
```js
...
#### <a name="type-field-merge-resolvers"></a>FieldMergeResolver [✯](#contents)
```js
...
#### <a name="type-union-merge-resolvers"></a>UnionMergeResolver [✯](#contents)
```js
